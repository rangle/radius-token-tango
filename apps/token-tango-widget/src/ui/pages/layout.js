...
export const PageLayout = ({ synched, name, error, appVersion, synchDetails, openConfig, children, }) => {
    const synchMetadata = synchDetails && synchDetails[2];
    return (<AutoLayout name="WidgetFrame" effect={{
            type: "drop-shadow",
            color: "#00000040",
            offset: {
                x: 0,
        ...
        }} fill="#F6F6F6" stroke="#858585" cornerRadius={6} direction="vertical" spacing={16} padding={24}>
      <WidgetHeader>
        <AutoLayout name="Header" strokeWidth={1.157} overflow="visible" direction="vertical" spacing={17} verticalAlignItems="center" horizontalAlignItems="center">
          <AutoLayout name="Frame 1000002077" overflow="visible" direction="vertical" spacing={8} verticalAlignItems="center" horizontalAlignItems="center">
            <SVG name="Union" height={35} width={23} src="<svg width='25' height='35' viewBox='0 0 25 35' fill='none' xmlns='http://www.w3.org/2000/svg'>
...
"/>
            <Frame name="token tango" strokeWidth={0} overflow="visible" width={184.987} height={27.52}>
              <SVG name="Vector_Vector_Vector_Vector_Vector_Vector_Vector_Vector_Vector_Vector" height={28} width={185} src="<svg width='185' height='28' viewBox='0 0 185 28' fill='none' xmlns='http://www.w3.org/2000/svg'>
...
"/>
          ...
          <Text name="Want to export your Figma Variables as Tokens? No problem! Validate naming conventions and export directly to Github." fill="#696969" width={421} verticalAlignText="center" horizontalAlignText="center" lineHeight="130%" fontFamily="Inter" fontSize={14} fontWeight={700}>
            Want to export your Figma Variables as Tokens? No problem! Validate
            naming conventions and export directly to Github.
          </Text>
        </AutoLayout>
      </WidgetHeader>
      {error && (<AutoLayout cornerRadius={6} padding={6} spacing={6} direction="vertical" fill="#fff">
          <Text fontSize={12} fill={"#ff0000"}>
            {error}
          </Text>
        </AutoLayout>)}
      {synchMetadata && (<RepositoryRibbon name={synchMetadata.name} avatarUrl={synchMetadata.lastCommits?.[0].autor_avatar_url} commitMessage={synchMetadata.lastCommits?.[0].message} dateTime={synchMetadata.lastCommits?.[0].author.date} userName={synchMetadata.lastCommits?.[0].author.name} version={synchMetadata.version} openConfig={openConfig} status={synched ? "online" : "disconnected"}/>)}
      <AutoLayout cornerRadius={12} padding={16} spacing={16} direction="vertical" horizontalAlignItems="center" fill="#fff" width="fill-parent">
        {children}
      </AutoLayout>
