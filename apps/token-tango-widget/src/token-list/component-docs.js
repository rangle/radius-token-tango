...
export const ComponentDocs = ({ usage, deleteComponent, isChildren, isDeleted, }) => {
    if (!usage)
    ...
    const [componentIcon, titleTextSize] = isChildren
        ? ["instance", 14]
        : ["component", 20];
    ...
    return (<AutoLayout name="Component-Frame" overflow="visible" direction="vertical" padding={{
      ...
      <AutoLayout name="Component-Header" stroke={"#aaa"} strokeWidth={1} overflow="visible" spacing="auto" width="fill-parent" height={32} verticalAlignItems="center" strokeDashPattern={[2, 2]}>
        <AutoLayout name="Title" overflow="visible" spacing={10} verticalAlignItems="center">
          ...
          <Text name="Component-title" fill="#030303" fontSize={titleTextSize} fontWeight={700}>
            {name}
          </Text>
          {isChildren ? (<Icon16px icon="close" onClick={() => deleteComponent(id)}/>) : (<></>)}
        </AutoLayout>
      ...
      <AutoLayout name="Component-Content" stroke={"#aaa"} strokeWidth={1} overflow="visible" direction="vertical" spacing={6} padding={{
            top: 6,
            right: 0,
            bottom: 0,
            left: 4,
        }} strokeDashPattern={[2, 2]}>
        {props.map((prop) => (<PropDocs prop={prop}/>))}

        {childrenToRender.length > 0 && (<AutoLayout name="Children-Component-Group" spacing={6}>
            {childrenToRender.map((subcomponent, key) => (<ComponentDocs key={key} usage={subcomponent} isChildren={true} deleteComponent={deleteComponent} isDeleted={isDeleted}/>))}
          </AutoLayout>)}
      </AutoLayout>
    </AutoLayout>);
};
