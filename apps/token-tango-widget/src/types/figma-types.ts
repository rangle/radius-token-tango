 ...
 * Type guard for Figma vector nodes
 */
export const isComponentSet = (node: SceneNode): node is ComponentSetNode => {
...
export const isComposite = (
  node: SceneNode,
): node is ComponentNode | InstanceNode | FrameNode | VectorNode | ComponentSetNode => {
  return isComponent(node) || isInstance(node) || isFrame(node) || isVector(node) || isComponentSet(node);
};